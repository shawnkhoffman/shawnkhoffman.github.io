<template>
  <nav role="navigation" aria-label="Main navigation" data-testid="main-navigation"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
        <div class="flex-shrink-0 flex items-center">
          <router-link to="/" :class="`text-xl font-bold text-base-content transform transition-all duration-300 ${isDrawerOpen ? 'translate-x-[-100%] opacity-0' : 'translate-x-0 opacity-100'
            }`" @click="() => handleLinkClick('Home')" aria-label="Home">
            shawnkhoffman.dev
          </router-link>
        </div>
        ...
        <div class="hidden lg:flex items-center space-x-4" role="list" aria-label="Desktop navigation">
          <div v-for="(link, index) in links" :key="index" class="relative">
            <template v-if="link.submenu">
              <details ref="aboutRef" class="group list-none" :open="isAboutOpen"
                ...
                <summary
                  ...
                  @click.prevent="toggleAboutMenu" role="button" :aria-expanded="isAboutOpen"
                  aria-controls="desktop-about-submenu">
                  {{ link.label }}
                  <Icon icon="fa6-solid:caret-down" :class="`ml-1 transition-transform duration-300 ${isAboutOpen ? 'rotate-180' : ''
                    } hidden lg:inline`" aria-hidden="true" />
                </summary>
                <Submenu :items="link.submenu" :is-open="isAboutOpen" :on-item-click="handleLinkClick" />
              </details>
            </template>
            <template v-else>
              <NavLink :href="link.href!" :label="link.label" :on-click="handleLinkClick" />
            </template>
          </div>
          <div class="relative">
            <ThemeController />
        ...
        <div class="flex items-center lg:hidden">
          <button
            ...
            @click="() => setIsDrawerOpen(!isDrawerOpen)" aria-label="Toggle mobile menu" :aria-expanded="isDrawerOpen"
    ...
    <MobileMenu :is-open="isDrawerOpen" :links="links" :on-link-click="handleLinkClick" :on-close="handleDrawerClose"
