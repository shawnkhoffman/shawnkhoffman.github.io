...
describe('Footer', () => {
  ...
  test('renders with copyright text', () => {
    ...
    expect(copyrightText?.includes(`© ${currentYear} Shawn Hoffman. All rights reserved.`)).toBe(true);
  });
  
  test('includes social links', () => {
    const { container } = render(<Footer />);
  ...
  test('tracks analytics when links are clicked', () => {
    const { container } = render(<Footer />);
